...
public final class HumanityInventoryUtil {
    private HumanityInventoryUtil() {
    }

    public static int countHumanity(@Nullable Inventory inventory) {
        if (inventory == null) {
            return 0;
        } else {
            int total = 0;
                ...
                if (section != null) {
                    ...
                    for(short slot = 0; slot < capacity; ++slot) {
                        ...
                        if (stack != null) {
                            ...
                            if (id != null && looksLikeHumanity(id)) {
    ...
    /**
     * Removes all soft humanity essence items from all inventory sections.
     * Returns the total quantity removed.
     */
    public static int removeHumanity(@Nullable Inventory inventory) {
    ...
    private static boolean looksLikeHumanity(String itemId) {
        String lower = itemId.toLowerCase();
        // Match soft humanity essence (auto-consume), but not concentrated (hard) version
        return lower.contains("humanity_essence") && !lower.contains("concentrated");
    }
}
